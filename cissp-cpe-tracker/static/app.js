...
/** Returns total minutes from a raw duration string, or null if unknown. */
function parseDurationMinutes(raw) {
  ...
  else                         { s = parseInt(raw, 10) || 0; }
  return s / 60;
}

/** Returns true if the entry is under 15 minutes and must be bundled. */
function isShortEntry(row) {
  const mins = parseDurationMinutes(row.duration);
  return mins !== null && mins < 15;
}

/**
 * Calculates bundled CPE hours for a set of short-duration rows.
 * Sums their minutes, applies standard rounding (nearest 0.25, min 0.25, max 40).
 * Returns 0 if the combined total is still under 15 minutes.
 */
function _bundledShortCPE(shortRows) {
  if (!shortRows.length) return 0;
  const totalMins = shortRows.reduce((sum, r) => sum + (parseDurationMinutes(r.duration) || 0), 0);
  if (totalMins < 15) return 0;
  return Math.min(40, Math.round((totalMins / 60) / 0.25) * 0.25);
}
  ...
  { id: "duration", label: "Duration",
    td: row => {
      ...
      const warn = (mins !== null && mins < 15)
        ? `<span class="dur-warn" title="Under 15 min — CPE hours for short entries cannot be counted individually. Use Export PDF to bundle all short entries and calculate combined CPE hours.">⚠</span>`
        : "";
      return `<td style="white-space:nowrap;font-size:12px;color:var(--text-muted)">${fmtDuration(row.duration)}${warn}</td>`;
    }},
  { id: "type",     label: "Type",
  ...
  tbody.innerHTML = rows.map(row => {
    const cells = visibleColOrder.map(id => {
      if (id.startsWith("vendor-")) {
        const vid = id.slice(7);
        const v = VENDORS[vid];
        const shortMins = parseDurationMinutes(row.duration);
        if (shortMins !== null && shortMins < 15) {
          return `<td data-col="${id}" style="text-align:center;color:var(--text-dim);font-size:12px" title="Under 15 min — CPE bundled in PDF Export">—</td>`;
        }
        if (!vendorAcceptsType(vid, row.type)) {
          return `<td data-col="${id}" style="text-align:center;color:var(--text-dim);font-size:12px" title="${v.name} only counts webinars">—</td>`;
        }
        const hrs = parseFloat(row.cpe_hours) || r.min;
        return `<td data-col="${id}" style="white-space:nowrap"><input class="editable vendor-hours-input" type="number" min="${r.min}" max="${r.max}" step="${r.step}" value="${hrs}" onchange="updateVendorHours('${row.id}',parseFloat(this.value))" style="width:60px"></td>`;
      }
...
async function _doExportPDF(rows) {
  try {
    const shortRows  = rows.filter(isShortEntry);
    const normalRows = rows.filter(r => !isShortEntry(r));
    const normalHours   = normalRows.reduce((sum, r) => sum + (parseFloat(r.cpe_hours) || 0), 0);
    const bundledMins   = shortRows.reduce((sum, r) => sum + (parseDurationMinutes(r.duration) || 0), 0);
    const bundledHours  = _bundledShortCPE(shortRows);
    const totalHours    = normalHours + bundledHours;
    const bundleInfo    = shortRows.length ? { rows: shortRows, totalMins: bundledMins, bundledHours } : null;
    const html = buildPDFHTML(rows, totalHours, window.location.origin, bundleInfo);
...
function buildPDFHTML(rows, totalHours, origin, bundleInfo) {
  ...
  const tocRows = rows.map((r, i) => {
    const short = isShortEntry(r);
    const hrsCell = short
      ? `<span style="color:#999">0.00</span> <span style="font-size:6pt;color:#aaa;font-style:italic">bundled</span>`
      : parseFloat(r.cpe_hours || 0).toFixed(2);
    return `
    <tr>
      <td class="n">${i + 1}</td>
      <td>${escHtml(r.title)}</td>
      <td style="font-size:7.5pt;color:#555">${escHtml((r.domains || r.domain || "").split("|")[0].trim())}</td>
      <td class="r h">${hrsCell}</td>
      <td class="d">${fmtDate(r.published_date)}</td>
    </tr>`;
  }).join("");
  const bundleTocRow = bundleInfo && bundleInfo.bundledHours > 0 ? `
    <tr style="background:#fffbf0">
      <td class="n" style="color:#c97d10">★</td>
      <td style="color:#c97d10;font-style:italic">Bundled Short Activities (${bundleInfo.rows.length} entries, ${Math.floor(bundleInfo.totalMins)}m total)</td>
      <td></td>
      <td class="r h" style="color:#c97d10">${bundleInfo.bundledHours.toFixed(2)}</td>
      <td></td>
    </tr>` : "";
  ...
  <div class="cover page">
    ...
    <table class="toc">
      ...
      <tbody>${tocRows}${bundleTocRow}</tbody>
      <tfoot><tr>
  ...
  const pages = rows.map((row, i) => {
    ...
    const shortNote = short
      ? `<div style="background:#fffbf0;border:1px solid #dfc87a;border-left:4px solid #c97d10;padding:9px 12px;margin-bottom:11px;font-size:8.5pt;color:#6b4a00">
           <strong>Bundled Entry</strong> — This activity is under 15 minutes and cannot be submitted individually.
           It is grouped with other short entries on the <em>Bundled Short Activities</em> page of this report.
           Individual CPE: 0.00
         </div>`
      : "";
    const cpeDisplay = short ? "0.00 (bundled)" : parseFloat(row.cpe_hours || 0).toFixed(2);
  ...
  <div class="page">
    ...
    <div class="meta-grid">
      <div class="meta-item"><div class="meta-lbl">Presenter</div>
        <div class="meta-val">${escHtml(row.presenter || "")}</div></div>
      ...
      <div class="meta-item"><div class="meta-lbl">CPE Hours</div>
        <div class="meta-val">${cpeDisplay}</div></div>
  ...
  // Bundled short-activities summary page
  const bundlePage = bundleInfo && bundleInfo.rows.length ? (() => {
    const bRows = bundleInfo.rows;
    const totalMins = bundleInfo.totalMins;
    const bHours = bundleInfo.bundledHours;
    const bTableRows = bRows.map((r, i) => `
      <tr>
        <td class="n">${i + 1}</td>
        <td>${escHtml(r.title)}</td>
        <td class="r">${fmtDuration(r.duration)}</td>
        <td class="d">${fmtDate(r.published_date)}</td>
      </tr>`).join("");
    const hTot = `${Math.floor(totalMins / 60)}h ${Math.round(totalMins % 60)}m`;
    return `
  <div class="page">
    <div class="ep-eyebrow">Bundled Short Activities Summary</div>
    <div class="ep-title">Bundled Activities — Combined CPE Calculation</div>
    <div class="ep-rule"></div>
    <div style="background:#fffbf0;border:1px solid #dfc87a;border-left:4px solid #c97d10;padding:12px 14px;margin-bottom:14px;font-size:9pt;color:#6b4a00;line-height:1.6">
      The following ${bRows.length} activities are each under 15 minutes and cannot be submitted individually
      per CPE vendor guidelines (minimum 15 min / 0.25 CPE per entry).
      Their durations are combined here and CPE hours calculated using the standard rounding rule
      (nearest 0.25 hr increment, minimum 0.25, maximum 40).
    </div>
    <table class="toc">
      <thead><tr>
        <th class="n">#</th><th>Title</th><th class="r">Duration</th><th>Date</th>
      </tr></thead>
      <tbody>${bTableRows}</tbody>
      <tfoot>
        <tr><td colspan="2" style="text-align:right">Combined duration</td>
            <td class="r">${hTot}</td><td></td></tr>
        <tr><td colspan="2" style="text-align:right;color:#c97d10;font-weight:700">Combined CPE Hours</td>
            <td class="r" style="color:#c97d10;font-weight:700">${bHours.toFixed(2)}</td><td></td></tr>
      </tfoot>
    </table>
    <div style="margin-top:14px;font-size:8pt;color:#777;line-height:1.6">
      <strong>Calculation:</strong> ${Math.floor(totalMins)} total minutes ÷ 60 = ${(totalMins/60).toFixed(2)} hours,
      rounded to nearest 0.25 = <strong>${bHours.toFixed(2)} CPE</strong>.${bHours === 0 ? " (Combined total still under 15 min — no CPE awarded.)" : ""}
    </div>
    <div class="footer">Cybersecurity CPE Tracker &mdash; Bundled Activities &mdash; ${escHtml(now)}</div>
  </div>`;
  })() : "";
