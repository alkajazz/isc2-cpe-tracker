...
# ---------------------------------------------------------------------------
# certifications field (v1.7)
# ---------------------------------------------------------------------------

class TestCertificationsField:
    def test_certifications_field_present(self):
        """v1.7 certifications field must be present and default to empty string."""
        row = st.add_entry(_entry())
        result = st.read_all()[0]
        assert "certifications" in result
        assert result["certifications"] == ""

    def test_certifications_can_be_updated(self):
        row = st.add_entry(_entry())
        st.update_entry(row["id"], {"certifications": "isc2|isaca"})
        updated = st.read_all()[0]
        assert updated["certifications"] == "isc2|isaca"

    def test_certifications_update_protected_from_id_change(self):
        """certifications is allowed but id is still protected."""
        row = st.add_entry(_entry())
        st.update_entry(row["id"], {"certifications": "comptia", "id": "evil"})
        updated = st.read_all()[0]
        assert updated["certifications"] == "comptia"
